...
func goldenDF() *Frame {
	nullableStringValuesFieldConfig := (&FieldConfig{
	...
	df := NewFrame("many_types",
		NewField("string_values", Labels{"aLabelKey": "aLabelValue"}, []string{
			"Go Min",
			"JS Min (for >= 64)",
			"0 / nil / misc",
			"JS Max (for >= 64)",
		...
		NewField("nullable_string_values", Labels{"aLabelKey": "aLabelValue", "bLabelKey": "bLabelValue"}, []*string{
			stringPtr("Grafana"),
			stringPtr("❤️"),
			nil,
			stringPtr("🦥"),
		...
		NewField("int8_values", nil, []int8{
			math.MinInt8,
			math.MinInt8,
			0,
			math.MaxInt8,
			math.MaxInt8,
		}).SetConfig((&FieldConfig{}).SetMin(0).SetMax(1)),
		NewField("nullable_int8_values", nil, []*int8{
		...
		NewField("int16_values", nil, []int16{
			math.MinInt16,
			math.MinInt16,
			0,
			math.MaxInt16,
		...
		NewField("nullable_int16_values", nil, []*int16{
		...
		NewField("int32_values", nil, []int32{
			math.MinInt32,
			math.MinInt32,
			1,
			math.MaxInt32,
		...
		NewField("nullable_int32_values", nil, []*int32{
		...
		NewField("int64_values", nil, []int64{
			math.MinInt64,
			minEcma6Int,
			1,
			maxEcma6Int,
		...
		NewField("nullable_int64_values", nil, []*int64{
		...
		NewField("uint8_values", nil, []uint8{
			0,
			0,
			1,
			math.MaxUint8,
		...
		NewField("nullable_uint8_values", nil, []*uint8{
			uint8Ptr(0),
			uint8Ptr(0),
			nil,
		...
		NewField("uint16_values", nil, []uint16{
			0,
			0,
			1,
			math.MaxUint16,
		...
		NewField("nullable_uint16_values", nil, []*uint16{
			uint16Ptr(0),
			uint16Ptr(0),
			nil,
		...
		NewField("uint32_values", nil, []uint32{
			0,
			0,
			1,
			math.MaxUint32,
		...
		NewField("nullable_uint32_values", nil, []*uint32{
			uint32Ptr(0),
			uint32Ptr(0),
			nil,
		...
		NewField("uint64_values", nil, []uint64{
			0,
			0,
			1,
			uint64(maxEcma6Int),
		...
		NewField("nullable_uint64_values", nil, []*uint64{
			uint64Ptr(0),
			uint64Ptr(0),
			nil,
		...
		NewField("float32_values", nil, []float32{
		...
		NewField("nullable_float32_values", nil, []*float32{
		...
		NewField("float32_values_nans", nil, []float32{
			float32(math.NaN()),
			float32(math.Inf(1)),
			1.0,
			float32(math.Inf(-1)),
		...
		NewField("nullable_float32_values_nans", nil, []*float32{
		...
		NewField("float64_values", nil, []float64{
		...
		NewField("float64_nans", nil, []float64{
			math.Inf(-1),
			math.NaN(),
			0,
			math.NaN(),
		...
		NewField("nullable_float64_values", nil, []*float64{
		...
		NewField("nullable_float64_values_nans", nil, []*float64{
		...
		NewField("nullable_bool_values", nil, []*bool{
			boolPtr(true),
			boolPtr(false),
			nil,
			boolPtr(true),
		...
		NewField("timestamps", nil, []time.Time{
			time.Unix(0, 0),
		...
		NewField("nullable_timestamps", nil, []*time.Time{
...
func TestEncode(t *testing.T) {
	...
	if !bytes.Equal(b, want) {
		...
		newDf, err := UnmarshalArrowFrame(want)
		if err != nil {
			t.Fatal("unable to create frame from encoded arrow file")
		}

		if diff := cmp.Diff(df, newDf, FrameTestCompareOptions()...); diff != "" {
...
func TestDecode(t *testing.T) {
	...
	newDf, err := UnmarshalArrowFrame(b)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEncodeAndDecodeDuplicateFieldNames(t *testing.T) {
	frame := NewFrame("frame_dup_field_names",
		NewField("Duplicate", nil, []bool{true, false}),
		NewField("Duplicate", nil, []bool{false, true}),
	)
	...
	decoded, err := UnmarshalArrowFrame(encoded)
	if err != nil {
		t.Fatal(err)
	}
	if diff := cmp.Diff(frame, decoded, FrameTestCompareOptions()...); diff != "" {
		t.Errorf("Result mismatch (-want +got):\n%s", diff)
	}
}

func TestFrameMarshalArrowRowLenError(t *testing.T) {
	f := NewFrame("unequal length fields",
		NewField("1", nil, []string{}),
		NewField("1", nil, []string{"a"}),
	)
...
func TestFromRecord(t *testing.T) {
	...
	if diff := cmp.Diff(df, got, FrameTestCompareOptions()...); diff != "" {
		t.Errorf("Result mismatch (-want +got):\n%s", diff)
	}
}
