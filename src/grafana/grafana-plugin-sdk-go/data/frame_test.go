...
func TestFrame(t *testing.T) {
	df := NewFrame("http_requests_total",
		NewField("timestamp", nil, []time.Time{time.Now(), time.Now(), time.Now()}).SetConfig(&FieldConfig{
			DisplayName: "A time Column.",
		}),
		NewField("value", Labels{"service": "auth"}, []float64{1.0, 2.0, 3.0}),
		NewField("category", Labels{"service": "auth"}, []string{"foo", "bar", "test"}),
		NewField("valid", Labels{"service": "auth"}, []bool{true, false, true}),
	)
...
func TestDeleteRow(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input: NewFrame("http_requests_total",
				NewField("timestamp", nil, []time.Time{
					time.Date(2020, 1, 2, 3, 4, 18, 0, time.UTC),
					time.Date(2020, 1, 2, 3, 4, 19, 0, time.UTC),
					time.Date(2020, 1, 2, 3, 4, 20, 0, time.UTC),
				}),
				NewField("value", Labels{"service": "auth"}, []float64{
					1.0,
					2.0,
					3.0,
				}),
				NewField("category", Labels{"service": "auth"}, []string{
					"foo",
					"bar",
					"test",
				}),
			...
			idx: 1,
			output: NewFrame("http_requests_total",
				NewField("timestamp", nil, []time.Time{
					time.Date(2020, 1, 2, 3, 4, 18, 0, time.UTC),
					time.Date(2020, 1, 2, 3, 4, 20, 0, time.UTC),
				}),
				...
				NewField("valid", Labels{"service": "auth"}, []bool{true, true}),
			),
		},
		{
			...
			input: NewFrame("http_requests_total",
				...
				NewField("valid", Labels{"service": "auth"}, []bool{
					true,
					false,
					true,
				}),
	...
	for i := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.wantPanic {
				...
				if diff := cmp.Diff(tt.output, tt.input, FrameTestCompareOptions()...); diff != "" {
...
func ExampleNewFrame() {
	...
	frame := NewFrame("Frame Name",
		NewField("Time", nil, []time.Time{aTime, aTime.Add(time.Minute)}),
		NewField("Temp", Labels{"place": "Ecuador"}, []float64{1, math.NaN()}),
		NewField("Count", Labels{"place": "Ecuador"}, []*int64{&anInt64, nil}),
	)
...
func ExampleFrame_tSDBTimeSeriesDifferentTimeIndices() {
	...
	frames := make([]*Frame, len(res.Series))
	for i, series := range res.Series {
		frames[i] = NewFrame(series.Name,
			NewField("time", nil, make([]time.Time, len(series.Points))),
			NewField(series.Name, series.Labels, make([]float64, len(series.Points))),
		)
...
func ExampleFrame_tableLikeLongTimeSeries() {
	...
	frame := NewFrameOfFieldTypes("Long", 0,
		FieldTypeTime,
		FieldTypeFloat64, FieldTypeFloat64,
		FieldTypeString,
	)
...
func TestStringTable(t *testing.T) {
	frame := NewFrame("sTest",
		NewField("", nil, make([]bool, 3)),
		NewField("", nil, make([]bool, 3)),
		NewField("", nil, make([]bool, 3)),
	)
	tests := []struct {
...
func TestDataFrameFilterRowsByField(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			frame: NewFrame("time_filter_test", NewField("time", nil, []time.Time{
				...
				NewField("floats", nil, []float64{
					1.0, 2.0, 3.0, 4.0,
				})),
			filteredFrame: NewFrame("time_filter_test", NewField("time", nil, []time.Time{
				time.Date(2020, 1, 2, 3, 4, 15, 0, time.UTC),
				time.Date(2020, 1, 2, 3, 4, 30, 0, time.UTC),
			}),
				NewField("floats", nil, []float64{
	...
	for i := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			tt.shouldErr(t, err)
			if diff := cmp.Diff(tt.filteredFrame, filteredFrame, FrameTestCompareOptions()...); diff != "" {
				t.Errorf("Result mismatch (-want +got):\n%s", diff)
			}
		})
	}
...
func TestJSON(t *testing.T) {
	...
	t.Run("json.Unmarshal and json.Marshal", func(t *testing.T) {
		...
		t.Run("Should create equal data", func(t *testing.T) {
			...
			for i, v := range f {
				if diff := cmp.Diff(frames[i], v, FrameTestCompareOptions()...); diff != "" {
					t.Fatal(diff)
				}
			}
		})
...
func TestFrameFieldIndexByName(t *testing.T) {
	frame := NewFrame("Frame Name",
		...
		NewField("Temp", nil, []float64{}),
		NewField("Count", nil, []*int64{}),
	)
