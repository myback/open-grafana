...
func TestCopyAtDoesNotMutatePointerVector(t *testing.T) {
	frameA := NewFrame("test", NewField("test", nil, []*float64{float64Ptr(1.0)}))
	rowLength, err := frameA.RowLen()
	require.NoError(t, err)
	frameB := NewFrame("test", NewField("test", nil, []*float64{nil}))
...
func TestCopyAtDoesNotMutateVector(t *testing.T) {
	frameA := NewFrame("test", NewField("test", nil, []float64{1.0}))
	rowLength, err := frameA.RowLen()
	require.NoError(t, err)
	frameB := NewFrame("test", NewField("test", nil, []float64{0.0}))
	for i := 0; i < rowLength; i++ {
		frameB.Set(0, i, frameA.Fields[0].CopyAt(i))
	}
	frameB.Set(0, 0, 2.0)
