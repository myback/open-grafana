...
func TestGoldenFrameJSON(t *testing.T) {
	...
	b, err := FrameToJSON(f, IncludeAll) // json.Marshal(f2)
	require.NoError(t, err)
	strF := string(b)

	b, err = ArrowBufferToJSON(a, IncludeAll)
	require.NoError(t, err)
	strA := string(b)
	...
	if diff := cmp.Diff(f, out, FrameTestCompareOptions()...); diff != "" {
...
type simpleTestObj struct {
	...
	FType  FieldType  `json:"type,omitempty"`
	FType2 *FieldType `json:"typePtr,omitempty"`
}
...
func TestFieldTypeToJSON(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, FieldTypeUnknown, v.FType)

	assert.Equal(t, `{"name":"hello"}`, string(b))

	ft := FieldTypeInt8

	v.FType = FieldTypeFloat64
	v.FType2 = &ft
	v.Name = ""
	b, err = json.Marshal(v)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	assert.Equal(t, FieldTypeInt8, v.FType)
	assert.Equal(t, FieldTypeTime, *v.FType2)
}
...
func TestFrame_UnmarshalJSON_SchemaOnly(t *testing.T) {
	f := NewFrame("test", NewField("test", nil, []int64{1}))
	d, err := FrameToJSON(f, IncludeSchemaOnly)
	require.NoError(t, err)
...
func TestFrameMarshalJSON_DataOnly(t *testing.T) {
	f := goldenDF()
	d, err := FrameToJSON(f, IncludeDataOnly)
	require.NoError(t, err)
	_, err = json.Marshal(testWrapper{Data: d})
	require.NoError(t, err)
	var newFrame Frame
...
func TestFrame_UnmarshalJSON_DataOnly(t *testing.T) {
	f := NewFrame("test", NewField("test", nil, []int64{}))
	d, err := FrameToJSON(f, IncludeDataOnly)
